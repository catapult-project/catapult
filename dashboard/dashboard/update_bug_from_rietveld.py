...
Rietveld issue: %(rietveld_issue_url)s
"""
...
# This is the special string that will occur in the bisect results if there
# are other possible culprit revisions that have a high confidence score.
_OTHER_REGESSIONS_STRING = 'Other regressions may have occurred'
...
class UpdateBugFromRietveldHandler(request_handler.RequestHandler):
  """URL endpoint for a cron job to update bugs after bisects."""
...
def _GetPerfTryResults(job):
  ...
  Returns:
    ...
    rietveld_issue_url for this bisect job, None if perf try job is pending or
    there's an error fetching run data.
  """
  results = {}
  # Fetch bisect bot results from Rietveld server.
  rietveld_issue_url = _RietveldIssueURL(job)
  response = _FetchURL(rietveld_issue_url)
  try_job_info = _ValidateRietveldResponse(response)

  results['buildbot_log_url'] = str(try_job_info['url'])
  results['rietveld_issue_url'] = str(rietveld_issue_url)
...
def _GetBisectResults(job):
  ...
  Returns:
    A dictionary containing status, results, buildbot_log_url, and
    rietveld_issue_url for this bisect job, note that the rietveld_issue_url key
    may contain a url for the buildbucket job and not necessarily rietveld. The
    name of the key has been kept for backwards compatibility. This will return
    None if bisect job is pending or there's an error fetching bisect run data.
  """
  results = {}
  # Fetch bisect bot results from Rietveld server.
  if job.use_buildbucket:
    try_job_info = _ValidateAndConvertBuildbucketResponse(
        buildbucket_service.GetJobStatus(job))
    issue_url = '/buildbucket_job_status/' + str(
        job.buildbucket_job_id)
  else:
    issue_url = _RietveldIssueURL(job)
    response = _FetchURL(issue_url)
    try_job_info = _ValidateRietveldResponse(response)

  results['buildbot_log_url'] = str(try_job_info['url'])
  results['rietveld_issue_url'] = str(issue_url)
...
def _PostSucessfulResult(job, bisect_results, issue_tracker):
  ...
  if bug and _ConfidenceAboveThreshold(bisect_results['results']):
...
def _ValidateAndConvertBuildbucketResponse(job_info):
  ...
  job_info['result'] = _BuildbucketStatusToStatusConstant(job_info['result'])
  return job_info
...
def _ConfidenceAboveThreshold(results_output):
  """Checks whether the confidence from the bisect job is considered high."""
  matches = re.findall(r'Estimated Confidence:\s(\d+(\.\d+)?)%',
                       results_output)
  if not matches or not matches[0]:
    return False
  confidence = float(matches[0][0])
...
def _GetAuthorsToCC(results_output):
  ...
  """
  # Don't CC anybody if the bisect outputs indicates that there are other
  # possible regressions with a high confidence score -- in this case the
  # main bisect culprit might be a false positive.
  if _OTHER_REGESSIONS_STRING in results_output:
    return []
...
def _RietveldIssueURL(job):
  rietveld_config = rietveld_service.GetDefaultRietveldConfig()
  return '%s/api/%d/%d' % (
      (rietveld_config.internal_server_url if job.internal_only
       else rietveld_config.server_url),
      job.rietveld_issue_id,
      job.rietveld_patchset_id)


def _BuildbucketStatusToStatusConstant(s):
