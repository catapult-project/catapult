...
def _ValidateToken(token, user):
  ...
  return xsrfutil.validate_token(_GetSecretKey(), token, user.user_id())


def GenerateToken(user):
  return xsrfutil.generate_token(_GetSecretKey(), user.user_id())
...
def TokenRequired(handler_method):
  ...
  def CheckToken(self, *args, **kwargs):
    ...
    if not user or not _ValidateToken(self.request.get('xsrf_token'), user):
      self.abort(403)
