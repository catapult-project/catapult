...
if six.PY2:
    ...
    def _OutputTableJSON(self, table_name):
      ...
      self.response.out.write(
          json.dumps({
              ...
              'values':
                  _GetRowValues(revisions, master_bot_pairs,
                                table_entity.tests),
              'units':
              ...
              'urls':
                  _GetDashboardURLMap(master_bot_pairs, table_entity.tests,
                                      rev_a, rev_b),
  ...
  def _UpdateNewestRevInMilestoneDict(bots, tests, milestone_dict):
    ...
    if bots and tests:
      test_path = bots[0] + '/' + tests[0]
      test_key = utils.TestKey(test_path)
      # Need to allow set this request as privileged in order to bypass
      # datastore hooks. This is okay here because table_config is internal_only
      # protected and will ensure that only the correct users can see
      # internal_only data.
  ...
  def _GetEndOfMilestone(rev, milestone_dict):
    ...
    for _, value_tuple in milestone_dict.items():
      if value_tuple[0] <= int(
          rev) < value_tuple[1]:  # 'rev' is a beginning rev.
  ...
  def _GetEndRevOrCurrentMilestoneRevs(rev_a, rev_b, milestone_dict):
    ...
    return (rev_a or rev_b), _GetEndOfMilestone((rev_a or rev_b),
                                                milestone_dict)
  ...
  def _GetNavigationMilestones(rev_a_milestone, rev_b_milestone,
                               milestone_dict):
