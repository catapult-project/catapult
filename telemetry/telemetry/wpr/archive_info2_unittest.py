...
_DEFAULT_PLATFORM = archive_info2._DEFAULT_PLATFORM
...
class WprArchiveInfo2Test(unittest.TestCase):
  def setUp(self):
  ...
  def createArchiveInfo(
    ...
    for wpr_file in wpr_files:
      ...
      with open(os.path.join(self.tmp_dir, wpr_file), 'w') as f:
        f.write(archive_data)
    return archive_info2.WprArchiveInfo.FromFile(
  ...
  def testDownloadArchivesIfNeededAllNeeded(self):
    archive_info = self.createArchiveInfo()
    ...
    archive_info.DownloadArchivesIfNeeded()
  ...
  def testDownloadArchivesIfNeededNonDefault(self):
    ...
    cloud_storage_stub.SetCalculatedHashesForTesting(
        {os.path.join(self.tmp_dir, recording1): "dummyhash1",
         ...
         os.path.join(self.tmp_dir, recording3): "dummyhash3",
         os.path.join(self.tmp_dir, recording4): "dummyhash4",})
    archive_info.DownloadArchivesIfNeeded(target_platforms=['linux'])
    self.assertItemsEqual(cloud_storage_stub.downloaded_files,
                          [recording1, recording4])

  def testDownloadArchivesIfNeededNoBucket(self):
    archive_info = self.createArchiveInfo(cloud_storage_bucket=None)
    cloud_storage_stub = self.overrides.cloud_storage
    # Second hash doesn't match, need to fetch it.
    cloud_storage_stub.SetRemotePathsForTesting(
  ...
  def testWprFilePathForStoryDefault(self):
    archive_info = self.createArchiveInfo()
    self.assertEqual(
        archive_info.WprFilePathForStory(page1),
        os.path.join(self.tmp_dir, recording1))
    self.assertEqual(
        archive_info.WprFilePathForStory(page2),
        os.path.join(self.tmp_dir, recording2))
    self.assertEqual(
        archive_info.WprFilePathForStory(page3),
        os.path.join(self.tmp_dir, recording1))

  def testWprFilePathForStoryMac(self):
    archive_info = self.createArchiveInfo()
    self.assertEqual(archive_info.WprFilePathForStory(page1, 'mac'),
                     os.path.join(self.tmp_dir, recording1))
    self.assertEqual(archive_info.WprFilePathForStory(page2, 'mac'),
                     os.path.join(self.tmp_dir, recording2))
    self.assertEqual(archive_info.WprFilePathForStory(page3, 'mac'),
                     os.path.join(self.tmp_dir, recording3))

  def testWprFilePathForStoryWin(self):
    archive_info = self.createArchiveInfo()
    self.assertEqual(archive_info.WprFilePathForStory(page1, 'win'),
                     os.path.join(self.tmp_dir, recording1))
    self.assertEqual(archive_info.WprFilePathForStory(page2, 'win'),
                     os.path.join(self.tmp_dir, recording2))
    self.assertEqual(archive_info.WprFilePathForStory(page3, 'win'),
                     os.path.join(self.tmp_dir, recording2))

  def testWprFilePathForStoryAndroid(self):
    archive_info = self.createArchiveInfo()
    self.assertEqual(archive_info.WprFilePathForStory(page1, 'android'),
                     os.path.join(self.tmp_dir, recording1))
    self.assertEqual(archive_info.WprFilePathForStory(page2, 'android'),
                     os.path.join(self.tmp_dir, recording2))
    self.assertEqual(archive_info.WprFilePathForStory(page3, 'android'),
                     os.path.join(self.tmp_dir, recording5))

  def testWprFilePathForStoryLinux(self):
    archive_info = self.createArchiveInfo()
    self.assertEqual(archive_info.WprFilePathForStory(page1, 'linux'),
                     os.path.join(self.tmp_dir, recording1))
    self.assertEqual(archive_info.WprFilePathForStory(page2, 'linux'),
                     os.path.join(self.tmp_dir, recording2))
    self.assertEqual(archive_info.WprFilePathForStory(page3, 'linux'),
                     os.path.join(self.tmp_dir, recording4))

  def testWprFilePathForStoryBadStory(self):
    archive_info = self.createArchiveInfo()
    self.assertIsNone(archive_info.WprFilePathForStory(pageNew1))


  def testAddRecordedStoriesNoStories(self):
    archive_info = self.createArchiveInfo()
    old_data = archive_info._data.copy()
    archive_info.AddNewTemporaryRecording()
    archive_info.AddRecordedStories(None)
    self.assertDictEqual(old_data, archive_info._data)
  ...
  def testAddRecordedStoriesDefault(self):
    ...
    archive_info.AddNewTemporaryRecording(new_temp_recording)
    archive_info.AddRecordedStories([page2, page3])
  ...
  def testAddRecordedStoriesNotDefault(self):
    ...
    with open(new_temp_recording, 'w') as f:
      f.write('wpr data')
    archive_info.AddNewTemporaryRecording(new_temp_recording)
    archive_info.AddRecordedStories([page2, page3], target_platform='android')
  ...
  def testAddRecordedStoriesNewPage(self):
    ...
    with open(new_temp_recording, 'w') as f:
      f.write('wpr data')
    archive_info.AddNewTemporaryRecording(new_temp_recording)
    archive_info.AddRecordedStories([pageNew1])

    with open(new_temp_recording, 'w') as f:
      f.write('wpr data2')

    archive_info.AddNewTemporaryRecording(new_temp_recording)
    archive_info.AddRecordedStories([pageNew2], target_platform='android')
