...
class SharedPageStateTests(unittest.TestCase):
  ...
  def testBrowserStartupURLAndExtraBrowserArgsSetCorrectly(self):
    ...
    for p in (google_page, example_page, gmail_page):
      ...
      # Fake possible browser saves that finder_options that are passed into it.
      page_level_finder_options = shared_state._possible_browser.finder_options
      self.assertEqual(
          p.startup_url,
          page_level_finder_options.browser_options.startup_url)
      self.assertNotEqual(
          p.startup_url,
      ...
      for page_level_argument in p.extra_browser_args:
        self.assertIn(
            page_level_argument,
            page_level_finder_options.browser_options.extra_browser_args)
        self.assertNotIn(
            page_level_argument,
