...
class InspectorPageTest(tab_test_case.TabTestCase):
  ...
  def testCustomActionToNavigate(self):
    ...
    self._tab.ExecuteJavaScript2('document.getElementById("clickme").click();')
  ...
  def testGetCookieByName(self):
    self.Navigate('blank.html')
    self._tab.ExecuteJavaScript2('document.cookie="foo=bar"')
  ...
  def testScriptToEvaluateOnCommit(self):
    ...
    self.assertEquals(self._tab.EvaluateJavaScript2('foo'), 'bar')
