...
class LinuxBasedInterfaceIntegrationTest(unittest.TestCase):
  ...
  def testListProcesses(self):
    cmd = 'sleep 42s'
    sleep_proc = self.lbi.StartCmdOnDevice(cmd.split(' '))
    self.assertIsNone(sleep_proc.poll())
    procs = self.lbi.ListProcesses()
    sleep_proc.kill()

    # All we can really check is that processes are being parsed.
    self.assertTrue(procs)
    proc_names = [proc[1] for proc in procs]
    self.assertIn(cmd, proc_names)
  ...
  @decorators.Enabled('linux', 'chromeos')
  def testKillAllMatchingKillsMatchingProcs(self):
    def KillSleeps(cmd):
      if 'sleep 30s' in cmd:
        return True
      return False
    cmd = ['pid=""']
    cmd += ['sleep 30s & pid="$pid $!"'] * 10
    cmd += ['for pid in ${pids[*]}; do',
            '  wait $pid',
            'done',
    ]
    with self._PopulateTmpFileWithCommands(cmd) as file:
      # Start sleep cmd after pushing.
      tmp_file = self._createTempFile(self.lbi.path.basename(file.name))
      self.lbi.PushFile(file.name, tmp_file)
      proc = self.lbi.StartCmdOnDevice(['bash', tmp_file])
      try:
        # Try to kill sleep commands.
        self.assertLessEqual(10, self.lbi.KillAllMatching(KillSleeps))
  ...
  @decorators.Enabled('linux', 'chromeos')
  def testGetRemotePortGetsUnallocatedPort(self):
    stdout, _ = self.lbi.RunCmdOnDevice(['netstat', '-ant'])

    first_port = self.lbi.GetRemotePort()

    # Assert a new port has been allocated.
    # 1. It's not present before
    # 2. It's been put in the reserved list.
    # Note that this method does not actually use the port.
    self.assertNotIn(":" + str(first_port), stdout)
    self.assertIn(first_port, self.lbi._reserved_ports)


    # Calling it again should give a new port as well.
    second_port = self.lbi.GetRemotePort()
    self.assertNotEqual(first_port, second_port)
    self.assertNotIn(":" + str(second_port), stdout)
    self.assertIn(second_port, self.lbi._reserved_ports)

  @decorators.Enabled('linux', 'chromeos')
  def testIsHTTPServerRunningOnPort(self):
    # Start a simple server.
    rc, _, _ = self.lbi.RunCmdOnDeviceWithRC(['which', 'python3'])
    if rc:
      raise unittest.SkipTest('DUT does not support python')


    port = self.lbi.GetRemotePort()
    proc = self.lbi.StartCmdOnDevice(['python3', '-m', 'http.server',
                                      str(port)])
    if proc.poll() is not None:
      raise unittest.SkipTest('DUT could not start an HTTP server')

    try:
      for _ in range(5):
        rc, _, _ = self.lbi.RunCmdOnDeviceWithRC(['curl',
                                                  'localhost:%d' % port])
        if rc == 0:
          break
      else:
        raise unittest.SkipTest('HTTP Server could be not be started')
      self.assertTrue(self.lbi.IsHTTPServerRunningOnPort(port))
      self.assertFalse(self.lbi.IsHTTPServerRunningOnPort(port + 1))
      proc.terminate()
      self.assertFalse(self.lbi.IsHTTPServerRunningOnPort(port))
    finally:
      proc.terminate()
