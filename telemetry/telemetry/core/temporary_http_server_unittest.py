...
class TemporaryHTTPServerTest(unittest.TestCase):
  ...
  def _VerifyScriptInjectedInMainFrame(self, tab):
    tab.WaitForDocumentReadyStateToBeComplete()
    script_index, load_event_captured = tab.EvaluateJavaScript(
        '[window.index, window.load_event_captured]')
    self.assertEquals(script_index, 1)
    self.assertTrue(load_event_captured)

  def testInjectScript(self):
    options = options_for_unittests.GetCopy()
    browser_to_create = browser_finder.FindBrowser(options)
    with browser_to_create.Create() as b:
      b.Start()
      b.SetHTTPServerDirectories(
          util.GetUnittestDataDir(),
          os.path.join(util.GetUnittestDataDir(), 'script_to_inject.js'))
      t = b.tabs[0]
      t.Navigate(b.http_server.UrlOf('/inject_script_test.html'))
      # Check injected script runs in the main frame.
      self._VerifyScriptInjectedInMainFrame(t)
      # Check injected script doesn't run in the iframe.
      self.assertEquals(
          None,
          t.EvaluateJavaScript(
              'document.getElementById("subframe").contentWindow.window.index'))

  def _testRedirectWithInjectScript(self, page_path):
    options = options_for_unittests.GetCopy()
    browser_to_create = browser_finder.FindBrowser(options)
    with browser_to_create.Create() as b:
      b.Start()
      b.SetHTTPServerDirectories(
          ...
          os.path.join(util.GetUnittestDataDir(), 'script_to_inject.js'))
      t = b.tabs[0]
      t.Navigate(b.http_server.UrlOf(page_path))
      t.WaitForDocumentReadyStateToBeComplete()
      def IsTargetPageLoaded():
        return 'inject test' == t.EvaluateJavaScript('document.title')
      util.WaitFor(IsTargetPageLoaded, 10)
      # Check injected script runs in the redirected target page.
      self._VerifyScriptInjectedInMainFrame(t)

  def testRedirectByMetaRefresh(self):
    self._testRedirectWithInjectScript(
        '/meta_redirect_to_inject_script_test.html')

  def testRedirectByJavascript(self):
    self._testRedirectWithInjectScript(
        '/js_redirect_to_inject_script_test.html')
