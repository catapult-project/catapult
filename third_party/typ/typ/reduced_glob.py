...
class ReducedGlob:
    ...
    def __init__(self, pattern):
        """Args:
            pattern: A string containing the pattern to match. * characters can
                be escaped by surrounding them with square brackets, e.g. [*].
        """
    ...
    def _compute_substrings(self):
        ...
        self._substrings = [s.replace('[*]', '*') for s in self._substrings]
    ...
    def matchcase(self, name):
        ...
        for i, substr in enumerate(self._substrings):
            ...
            # starts with it. Otherwise, we could find a match later in the string,
            # which would implicitly add a * to the front of |pattern|.
            if i == 0 and substr_start_index != 0:
                return False

            # Similarly, the last substring is special since we need to ensure that
            # all characters in |name| were matched. Otherwise, we would implicitly
            # add a * to the end of |pattern|.
