...
tvcm.unittest.testSuite('tvcm.settings_test', function() {
  function assertSettingIs(expectedValue, key) {
    assertEquals(expectedValue, tvcm.Settings.get('my_key'),
  ...
  test('oldStyleSettingYieldsDefaultValue', function() {
    var storage = tvcm.Settings.getAlternativeStorageInstance();
    storage.setItem(tvcm.Settings.namespace_('key'), 'hello world');

    assertEquals('value', tvcm.Settings.get('key', 'value'));
  });

  test('setGetString', function() {
    var settings = new tvcm.Settings();
    settings.set('my_key', 'my_val');
    assertEquals('my_val', settings.get('my_key'));
    // tvcm.Settings() is a singleton
    assertEquals('my_val', tvcm.Settings().get('my_key'));
  });
