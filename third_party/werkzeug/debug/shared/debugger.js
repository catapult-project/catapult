docReady(() => {
  ...
  addEventListenersToElements(
      document.querySelectorAll('div.detail'), 'click',
...
function addToggleFrameTraceback(frames) {
  frames.forEach((frame) => {
    frame.addEventListener('click', () => {
      frame.getElementsByTagName('pre')[0].parentElement.classList.toggle(
          'expanded');
    });
  })
}
...
function initPinBox() {
  document.querySelector('.pin-prompt form')
      .addEventListener('submit', function(event) {
        ...
        fetch(`${document.location.pathname}?__debugger__=yes&cmd=pinauth&pin=${
                  pin}&s=${encodedSecret}`)
            .then((res) => res.json())
            .then(({auth, exhausted}) => {
              if (auth) {
                ...
                alert(`Error: ${
                    exhausted ? 'too many attempts.  Restart server to retry.' :
                                'incorrect pin'}`);
              }
            })
            .catch((err) => {
...
function promptForPin() {
  if (!EVALEX_TRUSTED) {
    const encodedSecret = encodeURIComponent(SECRET);
    fetch(`${document.location.pathname}?__debugger__=yes&cmd=printpin&s=${
        encodedSecret}`);
